...
export const createKitchen: RequestHandler = async (req, res) => {
    const { email, password, name, description, image, startTime, endTime, tags } = req.body;

    try {
        // Check if a kitchen with the same name or email already exists
        const existingKitchen = await Kitchen.findOne({ $or: [{ name }, { email }] });
        if (existingKitchen) {
            return res.status(400).json({ message: "Kitchen already exists!" });
        }

        // Create a new kitchen
        const newKitchen = new Kitchen({ email, password, name, description, image, startTime, endTime, tags });
        ...
        return res.status(500).json({ message: "An error occurred while creating kitchen" });
    }
};
...
export const updateKitchen: RequestHandler = async (req, res) => { 
    ...
    const kitchen = await Kitchen.findByIdAndUpdate(kitchenId, {name, description, image, startTime, endTime, tags }, {new: true})
